...
export class CartFoodController {
  ...
  async addOrUpdateFood(params: {
    cartId: number;
    foodId: number;
    userId: string;
    count: number;
  }): Promise<IAddOrUpdateCartFood> {
    const { cartId, foodId, userId, count } = params;
    try {
      const userData = await this.userService.getUserByUniqueInfo({
        id: userId,
      });
      ...
      if (count < 1) {
        return {
          ...
          errors: {
            cart: {
              path: 'cart',
              message: 'count number should be at least 1',
            },
          },
        };
      }
      ...
      await this.cartService.updateTotalPriceByCartId(cartId);
      return {
  ...
  async deleteFoodFromCart(params: {
    cartFoodId: number;
    userId: string;
    cartId: number;
  }): Promise<IDeleteCartFoodResponse> {
    ...
    try {
      ...
      const deletedCartFood = await this.cartFoodService.deleteCartFood({
        where,
      });
      if (!deletedCartFood) {
        return {
