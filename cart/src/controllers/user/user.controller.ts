...
export class UserController {
  ...
  async createUserForCart(data: IUserCreatedEvent): Promise<ICreateUserCart> {
    try {
      const foundUser = await this.userService.getUserByUniqueInfo({
        id: data.id,
      });
      if (foundUser) {
        return {
          ...
          data: null,
          errors: {
            user_cart: { path: 'user', message: 'user already exists' },
          },
        };
      }
      ...
      if (!user) {
        return {
          ...
          data: null,
          errors: {
            user_cart: { path: 'user', message: 'could not create user' },
          },
        };
      }
  ...
  async updateUserForCart(params: IUpdateUserEvent): Promise<IUpdateUserCart> {
    try {
