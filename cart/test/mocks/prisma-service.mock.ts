...
export const prismaServiceMock = {
  user: {
    findUnique: jest
      .fn()
      .mockImplementation((params: { where: Prisma.UserWhereUniqueInput }) => {
        const { where } = params;
    ...
    create: jest
      .fn()
      .mockResolvedValue({ id: '4', firstName: 'Skn', lastName: '1942' }),
    update: jest
      .fn()
      .mockImplementation(
        (params: {
  ...
  cart: {
    ...
    findUnique: jest.fn().mockResolvedValue({
      ...fakeCarts[0],
      CartFood: [{ ...fakeCartFoods[0], food: fakeFoods[0] }],
    }),
  ...
  food: {
    ...
    delete: jest
      .fn()
      .mockImplementation(
        async (args: { where: Prisma.FoodWhereUniqueInput }) => {
          const { where } = args;
          const foundFoodIndex = fakeFoods.findIndex(
            (food) => food.id === where.id,
          );
          if (foundFoodIndex === -1) {
            return null;
          }
  ...
  cartFood: {
    ...
    findUnique: jest
      .fn()
      .mockImplementation(
        (params: { where: Prisma.CartFoodWhereUniqueInput }) => {
          const { where } = params;
          const cartFood = fakeCartFoods.find(
            (cartFood) =>
              cartFood.foodId === where.foodInCart.foodId &&
              cartFood.cartId === where.foodInCart.cartId,
          );
          if (!cartFood) {
            return null;
          }
    ...
    delete: jest.fn().mockImplementation((args: Prisma.CartFoodDeleteArgs) => {
      const { where } = args;
      const index = fakeCartFoods.findIndex(
        (cartFood) => cartFood.id === where.id,
      );
      if (index === -1) {
        return null;
      }
