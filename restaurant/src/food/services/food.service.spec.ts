...
describe('FoodService', () => {
    ...
    it('should create food for existing restaurant', async () => {
        const newRestauarnt = await restaurantService.createRestaurant(createRestaurantData);
    ...
    it('should get food list', async () => {
        ...
        const filteredFoodList = await foodService.getFoodList({ name: newFood.name });
        expect(filteredFoodList).toHaveLength(1);

        const notExisitedFilteredFoodList = await foodService.getFoodList({ name: 'not exist' });
