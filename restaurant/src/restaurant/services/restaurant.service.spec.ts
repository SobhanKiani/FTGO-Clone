...
describe('RestaurantService', () => {
  ...
  it('should create restaurant', async () => {
    const newRestaurant = await restaurantService.createRestaurant(createRestaurantData);
  ...
  it('should return updated restaurant if restaurant exists', async () => {
    ...
    const updatedRestaurant = await restaurantService.updateRestaurant(newRestaurant.id, updateObj);
  ...
  it('should return null if the restaurant does not exists', async () => {
    const updateObj = { name: 'new name' };
    const updatedRestaurant = await restaurantService.updateRestaurant(-1, updateObj);
  ...
  it('should delete existing restaurant', async () => {
    const newRestaurant = await restaurantService.createRestaurant(createRestaurantData);
    const deletedRestaurant = await restaurantService.deleteRestaurant(newRestaurant.id);
  ...
  it('should get restaurants list', async () => {
    ...
    const filterByCat = await restaurantService.getRestaurants({ category: 'cat1' });
  ...
  it('should rate the restaurant', async () => {
    ...
    const updatedRestaurant = await restaurantService.getRestaurantById(newRestaurant.id);
